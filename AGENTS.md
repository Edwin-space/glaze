...
- Xcode project (`Glaze.xcodeproj`, generated by XcodeGen from `project.yml` — see `Tools/README.md`), not a bare Swift Package
- `Packages/GlazeCore` — platform-independent local Swift package (media/subtitle models, AI subtitle generation)
- `Sources/GlazeMac` — macOS app target source (AppKit/SwiftUI/VLC playback surface)
- SwiftUI app, Liquid Glass design system
- macOS 26 minimum
- Apple Silicon first
- Native VLC playback first, AVFoundation for media inspection/export
- Localized strings live under `Packages/GlazeCore/Sources/GlazeCore/Resources` and `Sources/GlazeMac/Resources`

Build command:

```bash
xcodegen generate && xcodebuild -project Glaze.xcodeproj -scheme GlazeMac -configuration Debug build
```
...
(`build_and_run.sh` runs `xcodegen generate` itself when `project.yml` changed, then `xcodebuild`, then launches the app — the command above is only for a build-only check.)
