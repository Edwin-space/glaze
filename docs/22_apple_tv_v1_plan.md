...
### Apple TV TV 앱 구조로 재정의 (2026-09-01)

Plex를 참고하던 초기 서버 목록 중심 진입을 폐기하고, Apple의 TV 앱이 사용하는 콘텐츠 중심 정보 구조로 통합했다. 이번 개편은 세 개의 별도 시안이 아니라 하나의 사용자 흐름이다.

1. **시네마틱 홈**: 이어보기와 최근 추가 영상을 먼저 보여준다. 소스가 없으면 서버 오류 대신 미디어 소스 연결 행동을 보여준다.
2. **개인 보관함**: 폴더와 영상을 16:9 셀프로 구분하고, 서버 선택은 내용과 분리된 `미디어 소스`에서 관리한다.
3. **네이티브 재생**: 영상을 채우고 시간축·10초 이동·자막을 일시적 기능 레이어로 올린다. 자막 상세는 화면 전체를 덮는 패널이 아니라 우측 하단 compact material로 한정한다.

탐색은 `검색 / 홈 / 보관함 / 미디어 소스 / 설정`의 transient Liquid Glass 사이드바를 사용한다. Menu/Back으로 사이드바를 열고 닫으며, 목적지를 선택하면 즉시 물러나 콘텐츠를 차단하지 않는다.

최초 실행은 `환영 → 기본 자막 언어 → 같은 네트워크의 미디어 소스`의 3단계다. 서버를 찾지 못해도 건너뛸 수 있고 나중에 `미디어 소스`에서 연결한다. 선택한 기본 자막 언어는 내장·외부 트랙 자동 선택에 쓰며, 설정에서 자동 선택·글자 크기·초기 설정을 다시 바꿀 수 있다.

실물 Apple TV 재설치는 사용자 승인 이후로 유보한다. 2026-09-01 기준 Apple TV 4K(3세대) tvOS 26.5 시뮬레이터에서 빌드, 온보딩 포커스 이동, 홈, 사이드바, 설정 화면을 우선 검증했다.
