...
- 네이티브 엔진 대상 파일은 AVFoundation 상세 검사를 재생 전 필수 단계로 보지 않는다. 대용량 MKV에서 검사가 오래 걸리면 원형 진행 표시가 고정되므로, 재생은 VLC에 즉시 위임하고 미디어 패널은 컨테이너 중심의 경량 정보를 먼저 보여준다.
- 드래그앤드롭 파일은 VLC 재생 수명 동안 security-scoped resource 접근을 유지한다.
- 같은 폴더 재생목록 스캔은 첫 재생 이후 백그라운드에서 확장하고, VLC playback instance는 앱 화면 진입 시 미리 생성한다.
- 파일 열기 이벤트는 pending URL이 남아 있을 때만 지연 재전송해 동일 파일이 0.4초 뒤 다시 로드되는 문제를 막는다.
- 로컬 파일 캐시 옵션을 낮췄지만 4K 대용량 MKV는 VLC 내부 디코더 초기화로 첫 프레임 전 약 2초대 검은 화면이 남을 수 있다.
- Native VLC surface는 `libvlc_media_player_new_from_media`를 매번 호출하지 않고, 재사용 media player에 `libvlc_media_player_set_media`로 새 media를 붙인다.
- 단일 파일 드롭 시 자동 재생목록 패널 표시를 중단해 첫 렌더 중 영상 표면 resize를 피한다.
...
- 6GB 이상 장편 MKV 실행 시 원형 진행 표시 고정 없이 첫 프레임 표시와 미디어 정보 패널 경량 정보 표시 확인.
- 선택 파일 즉시 재생 후 같은 폴더 재생목록이 백그라운드에서 확장되는 흐름 확인.
