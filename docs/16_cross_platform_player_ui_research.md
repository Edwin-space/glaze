...
재생 컨트롤은 Apple TV와 YouTube에서 이미 학습된 순서를 활용한다. `재생/일시정지 → 시간축 → 음량 또는 오디오 → CC → PiP → 전체 화면`을 기본으로 하되, iPhone에서는 시스템 음량 조절과 중복되는 상시 볼륨 슬라이더를 제거한다.
...
글레이즈 적용: 영상 하단의 단일 transient transport surface와 우측 action cluster를 유지한다. 자막 표시 전환은 CC에서 즉시 처리하고, 생성·불러오기·언어·품질 같은 준비 작업만 Inspector로 넘긴다.
...
- 영상처럼 시각적으로 풍부한 배경 위 transient controls에는 clear variant를 검토한다.
...
| 계약 | 공통 정의 | 플랫폼 표현 |
| --- | --- | --- |
| Player chrome | 재생, 시간축, ±10초, CC, PiP, 전체 화면 | Mac hover, iPad/iPhone tap |
...
- 영상 위 clear Liquid Glass와 설정 surface의 regular material 분리
