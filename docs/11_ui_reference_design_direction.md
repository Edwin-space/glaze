...
## 설치 앱 직접 감사: Movist Pro · Submarine Player (2026-08-28)

이 Mac에 설치된 두 앱을 같은 테스트 영상으로 직접 실행한 결과는 [경쟁 제품 감사](audits/2026-08-28_movist_submarine_competitor_audit.md)에 기록했다.

- Movist는 내장·외부·온라인·생성 자막을 소스로 구분하고, 트랙·싱크·표시·TTS를 일반 플레이어 문법 안에서 촘촘하게 제공한다.
- Submarine은 원본 언어, 번역 언어, 전사 모델, 번역 모델, 원문/번역 표시를 독립된 작업으로 명확하게 구분한다.
- Glaze는 영상 위에 모든 조작을 모으지 않는다. 감상 조작은 분산형 micro controls로 유지하고, 기존 자막 선택·언어 교정·번역·AI 신규 생성은 Inspector 안에서도 서로 다른 섹션으로 나눈다.
- 기존 자막이 있는데 AI 생성 버튼이 같은 행동처럼 보이는 구조는 금지한다. AI 생성에는 음성을 새로 분석한다는 설명과 명시적 시작 동작이 반드시 필요하다.
...
- 네트워크 미디어 브라우저는 Finder의 정보 구조를 따른다. 좌측에는 즐겨찾기·DLNA·WebDAV 위치를 고정하고, 우측 상단에는 뒤로 가기·현재 경로·즐겨찾기·보기 방식·새로고침을 한 줄에 둔다. 이 상단 바는 선택 전 빈 상태에서도 상세 영역 최상단에 붙고 세로 중앙에 떠 있지 않는다.
...
자막 Inspector는 다음 순서로 구성한다. AI 생성은 기존 자막이 없는 경우에 쓰는 마지막 독립 섹션이다.

1. 현재 선택된 자막과 소스별 목록: 외부 자막 파일 / 영상에 포함된 자막
2. 현재 자막 언어 교정과 번역 대상 언어
3. 준비된 기존 자막을 사용하는 번역
4. `AI로 자막 만들기`: 영상 언어, 모델, 저장 위치, 진행률, 명시적 생성 액션
