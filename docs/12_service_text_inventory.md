...
| 설명(`help.*`) | 이름만으로는 뜻이 안 통하는 컨트롤 옆 `?`에서 열리는 설명 | 전사 모델이 뭔가요? |

### 설명(`help.*`) 작성 규칙 (2026-08-20)

인스펙터에는 사용자가 이미 아는 것이 아니라 **앱이 하는 일의 이름**을 붙일 수밖에 없는 항목이 있다 — 전사 모델, 컨테이너, 번역 품질처럼. 이름을 없앨 수 없다면 설명이 그 옆에 있어야 한다. `GlazeHelpButton`이 컨트롤 옆에 `?`를 두고 눌렀을 때 팝오버로 설명을 연다.

`.help()` 툴팁으로 대신하지 않는다. 마우스를 올려야만 뜨고, 있다는 사실 자체가 드러나지 않으며, 키보드로 접근할 수 없다.

작성 규칙:

- **무엇을 고르면 되는지까지 말한다.** 용어 정의만 쓰고 끝내지 않는다. "정확할수록 잘 안 들리는 대사를 더 잘 받아씁니다. 자막이 어색하면 한 단계 위로 올려보세요."
- 구현 용어를 쓰지 않는다. 프레임워크·코덱·API 이름은 설명 안에서도 등장하지 않는다.
- 팝오버 제목은 그 행의 라벨을 그대로 재사용한다. 행마다 문자열은 설명 하나만 추가된다.
- 뜻이 자명한 항목에는 붙이지 않는다(자막 표시, 길이 등). 모든 행에 `?`가 붙으면 아무 의미가 없다.

한 화면 안에 **같은 라벨이 두 번 나오면 그 자체가 결함이다.** 자막 패널의 "출력"이 사용 가능한 자막 상태와 번역 출력 형식 두 곳에 쓰이고 있어, 앞의 것을 "현재 자막"으로 고쳤다.
...
| `player.error.native_engine_unavailable` | 즉시 재생을 위한 네이티브 엔진 연결이 필요합니다. | Native engine integration is required for instant playback. | 오류 | 구현 |
