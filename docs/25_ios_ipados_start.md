...
- ~~**아이패드 하단에 탭 항목이 한 번 더 보인다.**~~ **2026-09-07 해소 — 앱의 문제가
  아니었다.** 스크린샷 도구가 화면을 겹쳐 캡처한 결과였다. `xcrun simctl io ...
  screenshot`으로 다시 찍으면 탭바는 상단에 한 번만 있다. 같은 원인으로 격자에
  표지가 희미하게 두세 번 겹쳐 보이던 것도 캡처 쪽 문제였다.
  → **아이패드 화면을 확인할 때는 `simctl io screenshot`을 쓴다**
...
1. ~~아이패드 탭바 중복 원인 찾기~~ (위 참고 — 캡처 도구 문제였다)
